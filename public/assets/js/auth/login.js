    ...
    if (otpForm && otpButton && resendOtpButton) {
        otpForm.addEventListener('submit', async (event) => {
            ...
            try {
                ...
                if (response.ok) {
                    showToast(result.message, 'success');
                    document.getElementById('reset-email').value = formData.get('email');
        ...
        if (isLoading) {
            ...
            button.innerText = button.dataset.originalText || 
                (button.id === 'login-button' ? 'Login' : 
                 button.id === 'forgot-password-button' ? 'Send OTP' : 
                 button.id === 'otp-button' ? 'Verify OTP' : 
                 button.id === 'resend-otp-button' ? 'Resend OTP' : 'Reset Password');
        }
    }
